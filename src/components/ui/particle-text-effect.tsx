...
export function ParticleTextEffect({
  ...
  const nextWord = (word: string, canvas: HTMLCanvasElement) => {
    ...
    const scale = Math.min(canvas.width / 8, 120);
    offscreenCtx.font = `700 ${scale}px "Noto Sans SC", sans-serif`;
  ...
  return (
    <div
      className={cn(
        ...
        variant === "glass"
          ? "rounded-3xl border border-blue-200/40 bg-white/70 p-4 backdrop-blur-xl shadow-lg"
          : "",
        className,
      )}
    >
      <canvas
        ref={canvasRef}
        className={cn(
          "h-auto w-full max-w-3xl drop-shadow-[0_30px_80px_rgba(59,130,246,0.35)]",
          canvasClassName,
        )}
